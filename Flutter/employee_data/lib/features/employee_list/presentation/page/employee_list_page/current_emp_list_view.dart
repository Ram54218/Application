...
class CurrentEmployeeListView extends StatelessWidget {
  const CurrentEmployeeListView({super.key});

  @override
  Widget build(BuildContext context) {
    context.read<EmployeeListBloc>().getCurrentEmployeeList();
    return BlocBuilder<EmployeeListBloc, EmployeeListState>(
      builder: (context, state) {
        if (state is CurrentEmployeeList) {
          return ListView.separated(
            itemCount: state.currentList.length,
            itemBuilder: (context, index) {
              final data = state.currentList[index];
              return GestureDetector(
                onTap: () {
                  context
                      .read<AddEmployeeBloc>()
                      .editEmployee(true, index, data);
                  Navigator.push(
                      context,
                      MaterialPageRoute(
                          builder: (context) =>
