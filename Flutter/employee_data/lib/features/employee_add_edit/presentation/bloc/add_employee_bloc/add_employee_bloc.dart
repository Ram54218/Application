...
class AddEmployeeBloc extends Cubit<AddEmployeeState> {
  ...
  void addEditEmployee(
      bool isCurrentList, bool isEdit, int index, EmployeeModel employeeModel) async {
    final employeeBox = EmployeeBoxes.getEmployeeDetails();
    if (isEdit) {
      List<EmployeeModel> allList;
      if(isCurrentList){
        allList = employeeBox.values
            .toList()
            .where((data) =>
  ...
  void editEmployee(bool isCurrentList, int index, EmployeeModel employeeModel) {
